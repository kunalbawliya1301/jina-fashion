...
async function getUploadSignature(token: string, resourceType: 'image' | 'video' = 'image'): Promise<CloudinarySignature> {
...
async function uploadToCloudinary(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (sig.paramsToSign) {
      ...
      if (!isVideo) formData.append('transformation', 'q_auto,f_auto')
    }
...
export default function Admin({ navigate }: Props) {
  ...
  // ── Social Media / Reels Context Hook & Upload State ───────────────────────
  const {
    items: socialItems,
    addItem: addSocialItem,
    updateItem: updateSocialItem,
    deleteItem: deleteSocialItem,
    resetToDefaults: resetSocialItems,
  } = useSocial()
  ...
  const [socialUploadFile, setSocialUploadFile] = useState<File | null>(null)
  const [socialUploadPreview, setSocialUploadPreview] = useState<string | null>(null)
  const [socialUploadProgress, setSocialUploadProgress] = useState(0)
  ...
  const handleOpenEditSocial = (item: any) => {
    ...
    setSocialForm({
      ...
      src: item.src,
      title: item.title,
      link: item.link || 'https://www.instagram.com/_jina_fashion',
    })
    setSocialUploadFile(null)
    setSocialUploadPreview(null)
    setSocialUploadProgress(0)
  ...
  const handleSocialFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (socialForm.type === 'video') {
      const allowedVideoTypes = ['video/mp4', 'video/webm', 'video/quicktime', 'video/x-m4v', 'video/mkv']
      if (!allowedVideoTypes.includes(file.type) && !file.type.startsWith('video/')) {
        showToast('Only MP4, WebM, MOV, or M4V video formats are allowed.', 'error')
        return
      }
      ...
      const allowedImgTypes = ['image/jpeg', 'image/png', 'image/webp', 'image/avif']
      if (!allowedImgTypes.includes(file.type)) {
        showToast('Only JPG, PNG, WebP, or AVIF images are allowed.', 'error')
        return
      }
      if (file.size > 15 * 1024 * 1024) {
        showToast('Image file must be under 15MB.', 'error')
        return
      }
    }

    setSocialUploadFile(file)
    setSocialUploadPreview(URL.createObjectURL(file))
  }

  const handleSocialSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!socialForm.src && !socialUploadFile) {
      showToast('Please select a media file to upload or enter a URL.', 'error')
      return
    }
    ...
    try {
      let finalSrc = socialForm.src

      if (socialUploadFile) {
        if (token) {
          try {
            const isVideo = socialForm.type === 'video'
            const sig = await getUploadSignature(token, isVideo ? 'video' : 'image')
            const { url } = await uploadToCloudinary(socialUploadFile, sig, pct => setSocialUploadProgress(pct))
            finalSrc = url
          } catch (err) {
            console.warn('Cloudinary upload unavailable, storing file in browser:', err)
            finalSrc = await readFileAsDataUrl(socialUploadFile)
          }
        } else {
          finalSrc = await readFileAsDataUrl(socialUploadFile)
        }
      ...
      if (!finalSrc) {
        showToast('Media source missing.', 'error')
        return
      }
      ...
      showToast((err as Error).message || 'Failed to save social item.', 'error')
    } finally {
  ...
  return (
    <div className="space-y-0 animate-fade-in min-h-screen">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5">
              {socialItems.map((item) => (
                <div key={item.id} className="group relative border border-border-custom rounded-2xl overflow-hidden bg-secondary-bg/40 aspect-[9/16] shadow-xs flex flex-col justify-between">
                  ...
                  {/* Top Badge */}
                  <div className="absolute top-2.5 left-2.5 bg-black/60 backdrop-blur-xs text-surface text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider z-10">
                    {item.type === 'video' ? '🎬 Video Reel' : '🖼️ Image'}
                  </div>

                  {/* Action Overlay */}
                  <div className="absolute inset-0 bg-primary/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-3 flex flex-col justify-between text-surface z-20">
                    <div className="flex items-center justify-end gap-1.5">
                      <button
                        onClick={() => handleOpenEditSocial(item)}
                        className="bg-secondary-bg/90 hover:bg-surface text-primary p-1.5 rounded-lg text-xs font-semibold transition-colors cursor-pointer"
                        title="Edit item"
                      >
                        ✏️
                      </button>
      ...
      {/* ── Add/Edit Social Media Reel / Image Modal ── */}
          ...
          <div className="bg-surface rounded-3xl border border-border-custom shadow-2xl max-w-md sm:max-w-lg w-full flex flex-col max-h-[85vh] sm:max-h-[88vh] my-auto overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 sm:py-5 border-b border-border-custom bg-surface shrink-0">
              <div>
                ...
                <h3 className="font-display text-xl sm:text-2xl font-semibold text-primary mt-0.5">
                  {socialEditingId ? 'Edit Social Reel / Media' : 'Add Social Reel / Media'}
                </h3>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSocialSubmit} className="flex flex-col overflow-hidden flex-1">
              <div className="p-5 sm:p-6 overflow-y-auto space-y-5 flex-1">
                {/* Media Type Toggle */}
                <div className="space-y-1.5">
                  <label className="block text-xs font-bold uppercase tracking-wider text-primary">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      type="button"
                      onClick={() => {
                        setSocialForm({ ...socialForm, type: 'video' })
                        if (socialUploadFile && !socialUploadFile.type.startsWith('video/')) {
                          setSocialUploadFile(null)
                          setSocialUploadPreview(null)
                        }
                      }}
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        setSocialForm({ ...socialForm, type: 'image' })
                        if (socialUploadFile && socialUploadFile.type.startsWith('video/')) {
                          setSocialUploadFile(null)
                          setSocialUploadPreview(null)
                        }
                    ...
                    >
                      <span>🖼️</span>
                      <span>Image Spread</span>
                    </button>
                  </div>
                </div>

                {/* Upload File Box / Dropzone */}
                <div className="space-y-3 pt-2 border-t border-border-custom/80">
                  <div className="flex items-center justify-between">
                    <label className="block text-xs font-bold uppercase tracking-wider text-primary">
                      {socialForm.type === 'video' ? 'Upload Video File' : 'Upload Image File'}
                    </label>
                    <span className="text-[10px] text-muted-custom">
                      {socialForm.type === 'video' ? 'MP4, WebM, MOV — max 100MB' : 'JPG, PNG, WebP — max 15MB'}
                    </span>
                  </div>

                  <div
                    onClick={() => socialFileInputRef.current?.click()}
                    className="group border-2 border-dashed border-border-custom hover:border-brand-accent/70 rounded-2xl p-4 text-center cursor-pointer transition-all bg-secondary-bg/20 hover:bg-secondary-bg/50 relative flex flex-col items-center justify-center min-h-[140px]"
                  >
                    {socialUploadPreview ? (
                      <div className="flex flex-col items-center gap-2 w-full">
                        <div className="relative group/media max-w-[180px] w-full aspect-[9/16] rounded-xl overflow-hidden shadow-md border border-border-custom bg-black/40">
                          {socialForm.type === 'video' ? (
                            <video
                          ...
                          <button
                            ...
                            className="absolute top-2 right-2 bg-rose-500 text-surface rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold shadow-md hover:bg-rose-600 transition-colors z-20 cursor-pointer"
                            title="Remove selected file"
                          >
                            ✕
                          </button>
                        </div>
                        <p className="text-[11px] text-brand-accent font-semibold truncate max-w-[240px]">
                          Selected: {socialUploadFile?.name}
                        </p>
                      </div>
                    ) : (
                      <div className="py-3 flex flex-col items-center">
                        <div className="w-12 h-12 rounded-2xl bg-surface border border-border-custom flex items-center justify-center text-muted-custom group-hover:text-brand-accent group-hover:border-brand-accent/40 shadow-xs transition-colors mb-2">
                          {socialForm.type === 'video' ? (
                            <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="1.75" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 10.5l4.72-4.72a.75.75 0 011.28.53v11.38a.75.75 0 01-1.28.53l-4.72-4.72M4.5 18.75h9a2.25 2.25 0 002.25-2.25v-9a2.25 2.25 0 00-2.25-2.25h-9A2.25 2.25 0 002.25 5.25v9a2.25 2.25 0 002.25 2.25z" />
                            </svg>
                          ) : (
                            <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth="1.75" viewBox="0 0 24 24">
                        ...
                        <p className="text-xs text-primary font-semibold">
                          Click to upload {socialForm.type === 'video' ? 'video file' : 'image file'}
                        </p>
                        <p className="text-[11px] text-muted-custom mt-0.5">
                          {socialForm.type === 'video' ? 'MP4, WebM, MOV, M4V — max 100MB' : 'JPG, PNG, WebP, AVIF — max 15MB'}
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <input
                    ...
                    type="file"
                    accept={socialForm.type === 'video' ? 'video/mp4,video/webm,video/quicktime,video/x-m4v,video/*' : 'image/jpeg,image/png,image/webp,image/avif'}
                    className="hidden"
                  ...
                  {isSocialUploading && (
                    <div className="space-y-1.5 pt-1">
                      <div className="flex items-center justify-between text-[11px] text-body-custom font-medium">
                        <span>Uploading {socialForm.type === 'video' ? 'video' : 'media'}...</span>
                        <span className="font-mono text-brand-accent font-bold">{socialUploadProgress}%</span>
                      </div>
                      <div className="h-2 bg-secondary-bg rounded-full overflow-hidden border border-border-custom/50">
                        <div className="h-full bg-brand-accent transition-all duration-200 rounded-full" style={{ width: `${socialUploadProgress}%` }} />
                      </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-1.5 pt-1">
                    <label className="block text-[10px] font-bold text-muted-custom uppercase tracking-wider">
                      Or direct {socialForm.type === 'video' ? 'Video' : 'Image'} URL:
                    </label>
                    <div className="flex items-center gap-2">
                      <input
                        type="url"
                        placeholder={socialForm.type === 'video' ? 'https://assets.mixkit.co/...video.mp4' : 'https://images.unsplash.com/...'}
                        value={socialForm.src}
                        onChange={(e) => {
                          setSocialForm({ ...socialForm, src: e.target.value })
                          setSocialUploadFile(null)
                          setSocialUploadPreview(null)
                        }}
                        className="flex-1 bg-secondary-bg/50 border border-border-custom rounded-xl px-3.5 py-2.5 text-xs text-primary focus:outline-none focus:border-brand-accent transition-all"
                      ...
                      {socialForm.src && (
                        <button
                          type="button"
                          onClick={() => setSocialForm({ ...socialForm, src: '' })}
                          className="px-2.5 py-2 text-[11px] font-semibold text-muted-custom hover:text-rose-500 bg-secondary-bg rounded-xl border border-border-custom transition-colors cursor-pointer shrink-0"
                        >
                          Clear
                        </button>
                  ...
                  {/* Active URL Preview (if URL exists and no new file selected) */}
                  {socialForm.src && !socialUploadPreview && (
                    <div className="flex items-center gap-3 p-2.5 bg-secondary-bg/30 border border-border-custom rounded-2xl">
                      <div className="w-10 h-16 rounded-xl border border-border-custom overflow-hidden shrink-0 bg-black/40">
                        {socialForm.type === 'video' ? (
                          <video src={socialForm.src} autoPlay loop muted playsInline className="w-full h-full object-cover" />
                        ) : (
                          <img src={socialForm.src} alt="Preview" className="w-full h-full object-cover" />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-brand-accent block">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-secondary-bg/50 border border-border-custom rounded-xl px-4 py-2.5 text-xs text-primary focus:outline-none focus:border-brand-accent transition-all"
                  />
                </div>
              ...
              <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border-custom bg-surface shrink-0">
                ...
                <button
                  type="submit"
                  disabled={isSocialUploading}
                  className="px-6 py-2.5 bg-brand-accent hover:bg-accent-hover disabled:opacity-60 disabled:cursor-not-allowed text-surface text-xs font-bold tracking-wider uppercase rounded-xl transition-all shadow-md cursor-pointer flex items-center gap-2"
                >
                  {isSocialUploading && (
                    <div className="w-3.5 h-3.5 border-2 border-surface/40 border-t-surface rounded-full animate-spin" />
                  )}
                  {isSocialUploading ? 'Uploading & Saving...' : socialEditingId ? 'Save Changes' : 'Save to Campaign'}
                </button>
              </div>
            </form>
          </div>
